...
from self_supervision.loaders.loader_3d import Loader3d
...
class GAN:
    ...
    def __init__(self):
        ...
        self.file_nums = 64
        self.channels = 1
        self.file_shape = (self.channels, self.file_rows, self.file_cols, self.file_nums)
        self.data_loader = Loader3d(shape=self.file_shape)
    ...
    def sample_train_images(self, epoch):
        ...
        fake = fake.detach().cpu().numpy().reshape(self.file_rows, self.file_cols, self.file_nums)
        imaging = imaging.detach().cpu().numpy().reshape(self.file_rows, self.file_cols, self.file_nums)
        edges = edges.detach().cpu().numpy().reshape(self.file_rows, self.file_cols, self.file_nums)
...
if __name__ == '__main__':
    model = GAN()
    # model.train(15000, 4, 100)
    model.transform_weights("C:\\Users\\David\\PycharmProjects\\KITS2021\\models\\gan\\18_05_2022_22_00")
